...
{
    public class EventBuffer<TKey, TValue> : LoggingReceiveActor
        where TKey : IComparable
    {
        ...
        public EventBuffer(GetKey keyExtractor)
        {
            ...
            Receive<TValue>(OnTValue);
            Receive<GetStats>(_ => OnGetStats());
            Receive<RemoveAndFindNext>(OnRemoveAndFindNext);
            Receive<FindNext>(OnFindNext);
            Receive<Buffer.DumpToActor>(OnDumpToActor);
            Receive<Buffer.DumpToStream>(_ => OnDumpToStream());
            Receive<Buffer.FeedToActor>(OnFeedToActor);
        ...
        private void OnFeedToActor(Buffer.FeedToActor finiteFeed)
        {
            if (_feedActor != null)
            {
        ...
        private void OnDumpToActor(Buffer.DumpToActor unroll)
        {
            foreach (var item in _buffer)
            {
        ...
        private void OnFindNext(FindNext findNext)
        {
            if (_buffer.Count == 0)
            {
        ...
        private void OnRemoveAndFindNext(RemoveAndFindNext removeAndFindNext)
        {
            if (_buffer.Count == 0)
            {
        ...
        private void OnTValue(TValue item)
        {
            var key = _keyExtractor(item);
            if (_feedingUpToKey != null && key.CompareTo(_feedingUpToKey) <= 0)
            {
                throw new Exception(
                    $"A new {typeof(TValue).Name} item was added to the {nameof(EventBuffer<TKey, TValue>)} while a feed was active, The added item has a smaller or equal key than the max. feed boundary ({_feedingUpToKey}).");
            }
